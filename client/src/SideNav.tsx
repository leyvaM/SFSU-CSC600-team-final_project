...
function SongsNav({ state, dispatch }: SideNavProps): JSX.Element {
  ...
  const handlePickRandomSong = () => {
    const randomIndex = Math.floor(Math.random() * filteredSongs.size);
    const randomSong = filteredSongs.get(randomIndex);
    if (randomSong) {
      dispatch(new DispatchAction("PLAY_SONG", { id: randomSong.get("id") }));
    }
  };
